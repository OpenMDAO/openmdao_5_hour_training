...
\begin{frame}
    ...
    \tikz[remember picture, overlay] \node[anchor=center] at ($(current page.center)-(5,1.12)$) {\includegraphics[scale=0.15]{images/frong2.png}};

    \tikz[remember picture, overlay] \node[anchor=center] at ($(current page.center)-(-5,1.12)$) {\includegraphics[scale=0.17]{images/front1.png}};
...
\begin{columns}[T]
    \column{0.5 \textwidth}
        ...
        \vspace{0.5cm}
            \uncover<2->{\item \small \textbf{Groups} organize the model and enable hierarchical solver strategies}
        \vspace{0.5cm}
            \uncover<3->{\item \small \textbf{Drivers} iteritively execute the model (optimizers and DOEs)}
        \vspace{0.5cm}
  ...
  \pause
    ...
    \only<4>{
        ...
        \begin{textblock*}{7cm}(8.5cm,2.2cm)
            \footnotesize \textcolor{red}{Define model inputs, outputs, and (optionally)\\partial derives using the setup() method.\\Called once before solve / optimization}
        \end{textblock*}
  }
...
\begin{columns}
    ...
    \column[T]{0.5\textwidth}
        ...
        \begin{tikzpicture}[overlay]
            \draw [red, ultra thick] (-4.75,2) circle (0.55cm);
      \end{tikzpicture}
    \end{columns}
...
\begin{frame}{Connecting multiple components}
    ...
    \only<1>{
        ...
        \begin{textblock*}{5cm}(8.3cm,7.2cm)
        ...
        \begin{tikzpicture}[overlay]        % Curly brace
            ...
            (6.2,2.) -- (6.2,3.5) node [midway,xshift=3cm] {\footnotesize $\includegraphics[scale=0.2]{images/explicit_box.png}$};
        \end{tikzpicture}
    }
...
\begin{frame}{\small Newton’s Method is simple in one dimension (Gradient Based!)}
    \begin{columns}
        \column[T]{0.5\textwidth}
                $$ x_{n+1} = x_{n} - \dfrac{f(x_{n})}{f'(x_{n})} $$                    % Equation
...
\begin{frame}{Lab \#2: Optimization of a cantilever beam}
	\begin{columns}
		\column[T]{.4 \textwidth}
			\begin{itemize}
				\item Find the thickness distribution for a cantilever beam that minimizes compliance subject to a volume constraint
...
\begin{frame}{Lab \#2: Simple FEM in 5 steps!}
	\begin{columns}
		\column[T]{.55 \textwidth}
			\begin{itemize}
				\item \small 0) Break the beam into segments (0 doesn't count as a real step!)
				\item \small 1) Compute each moment of inertia
				\item \small 2) Compute the local stiffness matrix for each segment
				\item \small 3) Combine all the local stiffness matrices into a global K matrix and solve the FEM
				\item \small 4) Pull displacements from the state vector and compute compliance
				\item \small 5) Compute beam volume
			\end{itemize}
	...
	\uncover<2->{
		\begin{tikzpicture}[overlay]
			\draw[red, thick, ->] (7.15, 1.5) to (12, 3.4);
			\draw[red, thick, ->] (7.15, 1.5) to (13.15, 3.7);
			\draw[red, thick] (.5, .7) to (7.1, .7);
			\draw[red, thick] (7.1, .7) to (7.1, 1.7);
			\draw[red, thick] (7.1, 1.7) to (.5, 1.7);
